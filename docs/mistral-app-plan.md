...
flowchart TB
    ...
    W -->|3. OCR 3| M1[Mistral OCR<br/>markdown + bbox]
    W -->|4. 構造化抽出| M2[Ministral 8B / Small<br/>JSON mode]
...
- **モデルを役割で使い分ける（これがコスト設計の肝）**
  - ページごとの単純抽出 → **Ministral 8B / Mistral Small**（安い、並列に叩く）
  - 横断的な質問・突合・計算 → **Magistral Small**（推論モデル、必要な時だけ）
  - 検索 → **mistral-embed** + pgvector
- **JSON スキーマを固定。** 構造化出力（JSON mode / スキーマ指定）で抽出結果の型を保証し、Zod でサーバ側でも再検証する。
...
- [ ] API キー取得、`@mistralai/mistralai` で OCR を 1 枚叩く
- [ ] 手書き文字・レシートの感熱紙・傾いたスキャンで劣化具合を見る
- [ ] bbox の座標系（正規化か絶対か、原点はどこか）を特定してメモに残す
...
- [ ] Next.js プロジェクト作成、環境変数に API キー
- [ ] アップロード → Blob 保存 → ジョブ登録
- [ ] Worker: OCR → Ministral で JSON 抽出 → DB 保存（bbox 込み）
- [ ] 結果を素の表で表示

### Phase 2 — 体験の核を作る（1.5 日）

- [ ] pdf.js ビューアを左ペインに設置
- [ ] bbox オーバーレイ描画、表のセルクリック → 該当ページへスクロール + ハイライト
- [ ] 複数ファイル / 複数ページ対応、進捗表示
...
| 項目 | 単価（目安） | 月額 |
|---|---|---|
| OCR 3（アノテーション付き） | $3 / 1,000 ページ | 約 $3 |
| 構造化抽出（Ministral 8B, 1 ページ 2k tokens 想定） | 小型モデル料金 | $1 未満 |
| 埋め込み | 微小 | $1 未満 |
...
| リスク | 影響 | 対策 |
|---|---|---|
| **日本語 OCR の精度が不足** | 致命的 | Phase 0 で先に潰す。ダメなら「英語の請求書」または「印字のみ」にターゲットを絞る |
| **bbox の座標系が UI と合わない** | ハイライトが機能しない | Phase 0 で座標系を確定。正規化座標 ↔ canvas 座標の変換を 1 箇所に閉じ込める |
